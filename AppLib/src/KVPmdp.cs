...
public class KVPmdp : KVP
{
    ...
    public KVPmdp(AppLib app, string group, string table = "KVP") : base(app, group)
    {
        ...
        sqliteUtil.RegisterField(new FieldDef(table, "KEY_ID", DataType.TEXT));
        sqliteUtil.RegisterField(new FieldDef(table, "GROUP_C", DataType.TEXT));
        ...
        List<Record> records = sqliteUtil.SelectRecords("SELECT KEY_ID, VALUE_X FROM "+table+" WHERE GROUP_C = '" + Group + "'");
        foreach (Record record in records)
        {
            string key = record.FieldVal("KEY_ID");
